...
@objective(zen, Max, sum(y[c,m,r]*prod_price[c,m] for c=1:C,m=1:M,r=1:R) 
                    ...
                    - sum(workers[b,m] for b=1:B, m=1:M)*3000)
...
#worker constraints
@constraint(zen, sum(workers[b,1] for b=1:B) == 4)
@constraint(zen, [m=2:M], sum(workers[b,m-1] for b=1:B) <= sum(workers[b,m] for b=1:B)) #Cannot have fewer workers in the next month than previous month
...
#37 þúsund fékk þorri


for b =1:B
    for m=1:M
        print(value(workers[b,m]), " ")
    end
    print("\n")
end
