...
@variable(zen, x[c=1:C,m=1:M] >= 0)

# Variable for sold products, months, regions
@variable(zen, y[c=1:C, m=1:M, r=1:R] >= 0)

# variable for stored products c at end of month m
@variable(zen, storage_level[c=1:C,m=1:M+1] >= 0)

# Machines
@variable(zen, machines[b=1:B,c=1:C, m=1:M] >= 0)
...
@constraint(zen,[c=1:C,m=1:M], storage_level[c,m] + x[c,m] >= sum(y[c,m,r] for r=1:R))
